...
describe('live quest progress', () => {
  ...
  test('recognizes destination-arrival transport quests as terminal goals', () => {
    const rule = buildLiveQuestProgressRule({
      gameId: 861,
      code: 'Bq3',
      text:
        '航路護衛任務：以兩艘航空戰艦 (或者補給艦)+ 任意船艦 * 4 為核心編成艦隊，在 1-6 實施運輸船隊護衛作戰並成功抵達目的地 2 次',
    })
  ...
  test('treats 1-6 completion wording as a terminal goal', () => {
    const rule = buildLiveQuestProgressRule({
      gameId: 807,
      code: 'B60',
      text:
        '旗艦「五十鈴改二」加上「皐月改二」「卯月改」+3 自由組成第三十一戰隊 (第一次) 完成一次 1-6，確保同航路的安全',
    })
  ...
  test('splits multi-map each-one S-rank sortie quests into native-like subgoals', () => {
    const rule = buildLiveQuestProgressRule({
      gameId: 889,
      code: 'B121',
      text:
