...
export const hasLiveQuestProgressEvidence = (
  record: LiveQuestProgressRecord | undefined,
) =>
  Boolean(
    record?.goals.some(
      (goal) => goal.count > 0 || goal.eventKeys.length > 0,
    ),
  )
...
const detectGoalKind = (
  ...
  if (
    /(?:到達|到达|抵達|抵达)(?:終點|终点|資源點|资源点|目的地)|(?:終點|终点|資源點|资源点|目的地)|ゴール(?:地点)?(?:へ|に)?(?:到達)?/u.test(
      text,
    )
  ) {
...
const detectMapSpecificGoalKind = (
  text: string,
  maps: Array<{ id: number; label: string }>,
): { kind: LiveQuestProgressGoalKind; rank?: LiveQuestProgressRank } | null => {
  if (maps.some((map) => map.id === 16) && /完成|通關|通关|クリア/u.test(text)) {
    return { kind: 'map_terminal' }
  }
  return null
}
...
const isDeferrableWeakBattleGoal = ({
  clause,
  goalKind,
  parsedMaps,
  required,
  ...
  clause: string
  goalKind: { kind: LiveQuestProgressGoalKind; rank?: LiveQuestProgressRank }
  parsedMaps: Array<{ id: number; label: string }>
  required: number
}) =>
  goalKind.kind === 'battle_boss_win' &&
  goalKind.rank === 'B' &&
  parsedMaps.length === 0 &&
  required === 1 &&
  !/BOSS|Boss|boss|B\s*(?:勝|胜)/u.test(clause)
...
export const buildLiveQuestProgressRule = ({
  ...
  let pendingWeakGoal: null | {
    kind: LiveQuestProgressGoalKind
    maps: Array<{ id: number; label: string }>
    rank?: LiveQuestProgressRank
    required: number
    separateByMap: boolean
  } = null
  ...
  const flushPendingWeakGoal = () => {
    if (!pendingWeakGoal) {
      return
    }
    addGoals(pendingWeakGoal)
    pendingWeakGoal = null
    pendingMaps = []
  }
  ...
  clauses.forEach((clause) => {
    ...
    if (parsedMaps.length) {
      flushPendingWeakGoal()
      pendingMaps = parsedMaps
    }

    const mapsForGoal = parsedMaps.length ? parsedMaps : pendingMaps
    const goalKind =
      detectGoalKind(clause) ?? detectMapSpecificGoalKind(clause, mapsForGoal)
